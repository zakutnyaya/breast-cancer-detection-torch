...
def split(
    ...
    gss = GroupShuffleSplit(
        test_size=.33, n_splits=n_splits, random_state=seed
    )
    for train_index, test_index in gss.split(X, y, groups):
        train_df.loc[train_index, "subset"] = "train"
        train_df.loc[test_index, "subset"] = "test"

    gss = GroupShuffleSplit(
        test_size=0.1, n_splits=n_splits, random_state=seed
    )
    train_df_sub = train_df[train_df['subset'] == "train"].reset_index().drop('index', axis=1)
    y_train = train_df_sub[['cancer']]
    X_train = train_df_sub[['image_id']]
    groups_train = train_df_sub['patient_id'].values

    for train_index, val_index in gss.split(X_train, y_train, groups_train):
        train_df_sub.loc[val_index, "subset"] = "val"

    train_df.loc[train_df['image_id'].isin(train_df_sub.image_id), 'subset'] = train_df_sub.subset.values
    return train_df
